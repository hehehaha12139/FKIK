...
void ASplineQuat::computeControlPoints(quat& startQuat, quat& endQuat)
{
	...
	//  Hint: use the SDouble, SBisect and Slerp to compute b1 and b2
		...
		// Calculate b1
		quat q1Prime = quat::SDouble(q_1, q0);
		quat q1Star = quat::SBisect(q1Prime, q1);
		quat b1 = quat::Slerp(q0, q1Star, 1.0 / 3.0);

		// Calculate b2
		quat q2Prime = quat::SDouble(q2, q1);
		quat q2Star = quat::SBisect(q2Prime, q0);
		quat b2 = quat::Slerp(q1, q2Star, 1.0 / 3.0);
...
quat ASplineQuat::getLinearValue(double t)
{
	...
	q = quat::Slerp(mKeys[segment].second, mKeys[segment + 1].second, t / 3.0 - segment);
	return q;
...
quat ASplineQuat::getCubicValue(double t)
{
	...
	q = quat::Scubic(b0, b1, b2, b3, t / 3.0 - segment);
