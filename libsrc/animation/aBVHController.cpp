...
void BVHController::update(double time)
{
	...
	root->setLocalTranslation(mRootMotion.getValue(t));
	for (int i = 0; i < skeleton->getNumJoints(); i++) 
	{
		int n = skeleton->getNumJoints();
		AJoint* j = skeleton->getJointByID(i);
		mat3 test = mMotion[i].getCachedValue(t).ToRotation();
		j->setLocalRotation(mMotion[i].getCachedValue(t).ToRotation()); 
	}
	skeleton->update();
}
