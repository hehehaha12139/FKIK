...
mat3 mat3::FromEulerAngles(RotOrder order, const vec3& anglesRad)
{
	...
	case YXZ:
		...
		double c = cos(anglesRad[VY]), s = sin(anglesRad[VY]);
		mat3 rotationY = mat3(vec3(c, 0.0, s),
							  vec3(0.0, 1.0, 0.0),
							  vec3(-s, 0.0, c));

		c = cos(anglesRad[VX]);
		s = sin(anglesRad[VX]);
		mat3 rotationX = mat3(vec3(1.0, 0.0, 0.0),
			vec3(0.0, c, -s),
			vec3(0.0, s, c));

		c = cos(anglesRad[VZ]);
		s = sin(anglesRad[VZ]);
		mat3 rotationZ = mat3(vec3(c, -s, 0.0),
			vec3(s, c, 0.0),
			vec3(0.0, 0.0, 1.0));
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
   ...
   double omega = acos(quat::Dot(q0, q1));
   double coef1 = sin((1 - u) * omega) / sin(omega);
   double coef2 = sin(u * omega) / sin(omega);
...
void quat::ToAxisAngle (vec3& axis, double& angleRad) const
{
	...
	double cosHalf = mQ[VW];
	angleRad = 2 * acos(cosHalf);
	axis = vec3(mQ[VX] / sin(acos(cosHalf)), mQ[VY] / sin(acos(cosHalf)), mQ[VZ] / sin(acos(cosHalf)));
}
...
void quat::FromAxisAngle (const vec3& axis, double angleRad)
{
	...
	vec3 a = axis;
	vec3 norAxis = a.Normalize();
	mQ[VW] = c; mQ[VX] = s * a[0]; mQ[VY] = s * a[1];  mQ[VZ] = s * a[2];
...
mat3 quat::ToRotation () const
{
	...
	double y = mQ[VY];
	double z = mQ[VZ];
	mat3 m = mat3(vec3(1 - 2 * pow(y, 2) - 2 * pow(z, 2), 2 * x * y - 2 * w * z, 2 * w * y + 2 * x * z),
		vec3(2 * w * z + 2 * x * y, 1 - 2 * pow(x, 2) - 2 * pow(z, 2), 2 * y * z - 2 * w * x),
		vec3(2 * x * z - 2 * w * y, 2 * w * x + 2 * y * z, 1 - 2 * pow(x, 2) - 2 * pow(y, 2)));
